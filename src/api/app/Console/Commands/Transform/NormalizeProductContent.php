...
class NormalizeProductContent extends Command
{
    ...
    public function handle()
    {
      ...
      // $product = Product::where('slug', 'rastitelnyy-kompleks-dlya-pecheni-s-vitaminami-hepato-complex-abu-60-kapsul-12401')->first();
      // $product = Product::where('slug', 'orthomol-osteo-ortomol-osteo-pri-osteoporoze-i-narusheniyakh-metabolizma-kostnoy-tkani-30-dney-poroshok-11863')->first();
      
      // $ru_content = $this->clearContent($product->old_content);
      // dd($ru_content);
      // // $ru_content = $product->old_content;
    ...
    private function clearContent($content) {
      ...
      if(@$dom->loadHTML(mb_convert_encoding("<div>$content</div>", 'HTML-ENTITIES', 'UTF-8'), LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD)) {
        ...
        foreach($xpath->query('//div/*') as $tag) {
          ...
          if(!$tag->hasChildNodes() || empty(trim(strip_tags($tag->textContent)))) {
