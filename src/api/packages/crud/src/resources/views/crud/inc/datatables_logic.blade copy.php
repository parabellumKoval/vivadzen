 ...
 @endphp
  ...
  <script type="text/javascript">
    jQuery(document).ready(function($) {
        // Move the DataTable configuration to a separate function
        window.crud.initializeDataTable = function(tableElement) {
            let tableId = $(tableElement).attr('id');
            let dtConfig = {...window.crud.dataTableConfiguration}; // Clone the configuration
            
            // Modify the ajax URL for nested tables if needed
            if (tableId !== 'crudTable') {
                let parentId = $(tableElement).data('parent-id');
                dtConfig.ajax.url = dtConfig.ajax.url + '?parent_id=' + parentId;
            }
            
            let dt = $(tableElement).DataTable(dtConfig);
            
            // If this is the main table, store it in crud.table
            if (tableId === 'crudTable') {
                window.crud.table = dt;
            ...
            // move search bar
            $("#" + tableId + "_filter").appendTo($('#datatable_search_stack'));
            $("#" + tableId + "_filter input").removeClass('form-control-sm');

            // override ajax error message
            $.fn.dataTable.ext.errMode = 'none';
            $(tableElement).on('error.dt', function(e, settings, techNote, message) {
                new Noty({
                    type: "error",
            ...
            $(tableElement).on('column-visibility.dt', function(event) {
                dt.responsive.rebuild();
            });
            ...
            dt.on('responsive-resize', function(e, datatable, columns) {
                if (dt.responsive.hasHidden()) {
                    $(tableElement).removeClass('has-hidden-columns').addClass('has-hidden-columns');
                } else {
                    $(tableElement).removeClass('has-hidden-columns');
                }
            });
            @else
        ...
        window.crud.initializeDataTable("#crudTable");
    });
    
    // Add function to initialize nested tables
    window.crud.initializeNestedTable = function(element) {
        return window.crud.initializeDataTable(element);
    };
</script>
