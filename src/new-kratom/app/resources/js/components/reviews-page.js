// Recenze page — all-products review list with filters, product filter,
// load-more pagination and a write-review modal (product selected from a list).
export default ({ apiBase = '/api/reviews', products = [] }) => ({
    products,
    items: [],
    page: 1,
    perPage: 9,
    ...
    get activeFilterCount() {
        return [this.verified, this.withPhotos, this.ratingFilter, this.productSlug]
    ...
    async fetchPage(replace) {
        ...
        try {
            ...
            if (this.productSlug) params.set('product', this.productSlug);
            ...
            this.hasMore = (meta.current_page || 1) < (meta.last_page || 1);
        } catch (e) {
            console.error(e);
        } finally {
            this.loading = false;
    ...
    openWriteModal(slug = '') {
        ...
        this.success = '';
        this.form = this.freshForm();
        this.form.product = slug || this.productSlug || '';
        this.modalOpen = true;
    },
    ...
    async submitReview() {
        ...
        if (!this.form.product) {
            this.error = 'Vyberte prosím produkt.';
            return;
        }
        ...
        try {
            ...
            fd.append('product', this.form.product);
            ...
            this.form = this.freshForm();
            setTimeout(() => { this.modalOpen = false; this.success = ''; }, 2200);
        } catch (e) {
