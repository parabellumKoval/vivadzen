...
class Order extends Mailable
{
    ...
    public function prepareData() {
      if($this->order->info['payment'] && count($this->order->info['payment'])) {
        ...
        $this->payment = "Статус: " . __('status.pay_status.'.$this->order->pay_status);
      ...
      if($this->order->info['delivery'] && count($this->order->info['delivery'])) {
        ...
        $delivery_arr = [
          ...
          isset($delivery['house']) && !empty($delivery['house'])? 'дом ' . $delivery['house']: null,
          isset($delivery['room']) && !empty($delivery['room'])? 'кв. ' . $delivery['room']: null,
          isset($delivery['zip']) && !empty($delivery['zip'])? 'индекс ' . $delivery['zip']: null
        ];

        $delivery_arr  = array_filter($delivery_arr);
        
        $this->delivery = "Статус: " . __('status.delivery_status.'.$this->order->delivery_status);
        $this->delivery .= ' / ';
        $this->delivery .= "Cпособ: " . __('validation.values.delivery.method.' . $delivery['method']);
      ...
      if($this->order->status && $this->order->price) {
        $this->common = "Статус: " . __('status.status.'.$this->order->status);
