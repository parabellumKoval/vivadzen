...
const { data, error, pending } = await useAsyncData(
  ...
  async () => {
    const response = await useArticleStore().index({ per_page: 12, page: page.value })
    return response
  },
  { server: true },
)
...
.kratom-page-shell { padding-top: 32px; }
.kratom-page-hero { margin-bottom: 28px; padding: 28px; border-radius: 32px; background: linear-gradient(135deg, rgba(255,247,236,.96), rgba(240,231,219,.92)); border: 1px solid rgba(74,91,68,.1); }
...
.kratom-page-hero__title { margin-bottom: 12px; font-size: 48px; line-height: .98; }
.kratom-page-hero__text { max-width: 700px; color: #5f6458; line-height: 1.7; }
.kratom-articles-grid { display: grid; gap: 20px; @include tablet { grid-template-columns: repeat(2, minmax(0, 1fr)); } @include desktop { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
